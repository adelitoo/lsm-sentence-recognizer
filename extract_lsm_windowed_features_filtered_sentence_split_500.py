"""
LSM Windowed Feature Extraction with SENTENCE-LEVEL SPLIT (500 Sentences)

This version ensures TRUE GENERALIZATION TESTING by splitting at the sentence level,
not the sample level. Works with the 500-sentence dataset generated with Chatterbox TTS.

Differences from regular version:
- Splits unique sentence IDs first (80 train, 20 test) = 400 train, 100 test
- Test set contains sentences NEVER seen during training
- Uses balanced split to maximize word coverage
"""
...
def load_spike_dataset(filename="sentence_spike_trains_500.npz"):
    """Load spike train dataset"""
    print(f"Loading '{filename}'...")
    data_path = Path(filename)
    if not data_path.exists():
        print(f"❌ Error: Dataset '{filename}' not found.")
        print("Please run 'audio_encoding_500.py' first.")
        return None, None
...
def split_by_sentence(X_spikes, y_labels, test_size=0.2, random_state=42):
    ...
    Args:
        X_spikes: All spike train samples (600 samples × 6 per sentence)
        y_labels: Sentence IDs (0-99)
        test_size: Fraction of sentences to put in test (default 0.2 = 20 sentences)
        random_state: Random seed for reproducibility

    Returns:
        X_train, X_test, y_train, y_test with sentence-level split
    """
    print("\n" + "="*60)
    ...
    # Count samples per sentence to verify consistency
    samples_per_sentence = []
    for sid in unique_sentence_ids:
        count = np.sum(y_labels == sid)
        samples_per_sentence.append(count)

    print(f"Samples per sentence: min={min(samples_per_sentence)}, max={max(samples_per_sentence)}, avg={np.mean(samples_per_sentence):.1f}")

    # Load BALANCED split sentence IDs (ensures better word coverage)
    balanced_split_file = "balanced_sentence_split_500.npz"
    if Path(balanced_split_file).exists():
        ...
        print(f"   Run 'python create_balanced_sentence_split_500.py' for better results")
        # Split sentence IDs (not samples!) - RANDOM fallback
    ...
    print(f"\nExample test sentence IDs (completely unseen during training):")
    print(f"  {sorted(test_sentence_ids)[:10]}...")
...
def main(multiplier: float, window_size: int, stride: int, variance_threshold: float):
    ...
    # Load spike dataset
    X_spikes, y_labels = load_spike_dataset(filename="sentence_spike_trains_500.npz")
    if X_spikes is None:
        return
    ...
    # Save with different filename to distinguish from sample-level split
    output_file = "lsm_windowed_features_filtered_sentence_split_500.npz"
    ...
    np.savez_compressed(
        ...
        split_type='sentence_level_500'  # Mark this as sentence-level split (500 sentences)
    )

    print("\n" + "="*60)
    print("✅ SENTENCE-LEVEL SPLIT FEATURE EXTRACTION COMPLETE! (500 SENTENCES)")
    print(f"Saved to: {output_file}")
    print("\n🎯 IMPORTANT: Test set contains COMPLETELY UNSEEN sentences!")
    print("   This tests TRUE GENERALIZATION to new vocabulary.")
    print("   Dataset: 400 train sentences, 100 test sentences")
    print("\nNext step:")
    print("  python train_ctc_500.py  # Train on 500-sentence dataset")
    print("="*60 + "\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract filtered windowed features with SENTENCE-LEVEL split for true generalization testing."
    )
    parser.add_argument(
        "--multiplier",
        type=float,
    ...
    print("\n" + "="*60)
    print("SENTENCE-LEVEL SPLIT FEATURE EXTRACTION")
