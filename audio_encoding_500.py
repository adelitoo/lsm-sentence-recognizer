...
def create_dataset(n_filters: int, filterbank: str):
    ...
    BASE_DATASET_PATH = Path("sentences_500")
    ...
    print("="*60)
    print("CREATING 500-SENTENCE DATASET (from CSV)")
    print("="*60)

    # --- Load Metadata ---
    if not METADATA_FILE.exists():
        print(f"ERROR: Metadata file not found.")
        print(f"Please ensure '{METADATA_FILE.resolve()}' exists")
        print("="*60)
        return
    ...
    output_filename = "sentence_spike_trains_500.npz"
    np.savez_compressed(output_filename, X_spikes=X_spikes, y_labels=y_labels)

    # Save label mapping
    label_map_filename = "sentence_label_map_500.txt"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create a spike train dataset from 500 sentence audio files.")
