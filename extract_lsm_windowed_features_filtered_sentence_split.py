"""
LSM Windowed Feature Extraction with SENTENCE-LEVEL SPLIT

This version ensures TRUE GENERALIZATION TESTING by splitting at the sentence level,
not the sample level. All augmented versions of a sentence stay together in either
train or test set, ensuring the test set contains completely unseen sentences.

Differences from regular version:
- Splits unique sentence IDs first (80 train, 20 test)
- All 6 augmented versions of each sentence stay together
...
def split_by_sentence(X_spikes, y_labels, test_size=0.2, random_state=42):
    ...
    if Path(balanced_split_file).exists():
        ...
        print(f"   Run 'python create_balanced_sentence_split.py' for better results")
...
def main(multiplier: float, window_size: int, stride: int, variance_threshold: float):
    ...
    output_file = "lsm_windowed_features_filtered_sentence_split.npz"
    ...
    np.savez_compressed(
        ...
        split_type='sentence_level'  # Mark this as sentence-level split
    )

    print("\n" + "="*60)
    print("✅ SENTENCE-LEVEL SPLIT FEATURE EXTRACTION COMPLETE!")
    print(f"Saved to: {output_file}")
    print("\n🎯 IMPORTANT: Test set contains COMPLETELY UNSEEN sentences!")
    print("   This tests TRUE GENERALIZATION to new vocabulary.")
    print("\nNext step:")
    print("  python train_ctc.py  # Will load sentence-split features automatically")
    print("  OR modify train_ctc.py to load from this specific file")
    print("="*60 + "\n")
